//
//  VinylConstants.m
//  VinylMap
//
//  Created by Linda NG on 11/24/15.
//  Copyright © 2015 Toaster. All rights reserved.
//

#import <Foundation/Foundation.h>

NSString *const DISCOGS_CONSUMER_KEY = @"VrBSOcWzbUsqcUoFFecY";
NSString *const DISCOGS_CONSUMER_SECRET = @"vWfYNMSqHwtXmJGDSUSEZmyfFOjRtjpY";
NSString *const DISCOGS_KEYCHAIN = @"discogsTokenSecret";
NSString* const DISCOGS_LOGIN_NOTIFICATION = @"DISCOGS_LOGIN_NOTIFICATION";

NSString *const DISCOGS_USERNAME = @"jason.harris@flatironschool.com";
NSString *const DISCOGS_PASSWORD = @"password";

NSString *const FACEBOOK_KEY = @"412777025588393";
NSString *const FACEBOOK_SECRET = @"f7152012f693ef67375c8e3237357c20";
NSString *const FACEBOOK_CLIENT_TOKEN = @"93bb2105153b8172a8874412eac0316e";
NSString *const FACEBOOK_CALLBACK = @"https://auth.firebase.com/v2/<amber-torch-8635.firebaseio.com>/auth/facebook/callback";

NSString *const FIREBASE_URL = @"https://amber-torch-8635.firebaseio.com/";
NSString *const FIREBASE_SECRET = @"CGGwgr3urUV02Nct06PEUD2XV6dni8T55VkIifAx";
NSString *const FIREBASE_DUMMY_USERNAME = @"fake@fake.com";
NSString *const FIREBASE_DUMMY_PASSWORD = @"password";
NSString *const FIREBASE_CHATROOM = @"https://amber-torch-8635.firebaseio.com/data/chatrooms";
